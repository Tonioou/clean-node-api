...
const makeAddAccount = (): AddAccount => {
  class AddAccountStub implements AddAccount {
    async add (account: AddAccountModel): Promise<AccountModel> {
      const fakeAccount = makeFakeAddAccount()
...
describe('SignUp Controller', () => {
  ...
  test('Should call EmailValidator with correct email', async () => {
    ...
    expect(isValidSpy).toHaveBeenCalledWith('any_email@sample.com')
  })
  ...
  test('Should return 500 if EmailValidator throws', async () => {
    ...
    const sut = new SignUpController(emailValidatorStub, addAccount, validation)
    const httpResponse = await sut.handle(makeFakeRequest())
  ...
  test('Should return 200 if valid data is provided', async () => {
    const { sut } = makeSut()
    const httpResponse = await sut.handle(makeFakeRequest())
    expect(httpResponse).toEqual(ok(makeFakeAddAccount()))
  })
